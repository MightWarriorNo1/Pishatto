...
const CastRegisterDirectPage: React.FC = () => {
    ...
    React.useEffect(() => {
        ...
        if (lineIdFromUrl) {
            ...
            sessionStorage.setItem('cast_line_data_direct', JSON.stringify({
    ...
    const handleLineLoginClick = () => {
        ...
        sessionStorage.setItem('cast_register_direct_form_data', JSON.stringify(formData));
        ...
        handleLineLogin({
            ...
            castDirectRegistration: true, // Flag for direct registration (no approval)
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            // Get CSRF token
            const csrfToken = await getCsrfToken();
            const response = await fetch(API_ENDPOINTS.CAST_REGISTER_DIRECT, {
                method: 'POST',
                headers: {
                    'Accept': 'application/json',
                    ...(csrfToken && { 'X-CSRF-TOKEN': csrfToken }),
            ...
            <div className="mx-4 mb-6">
                <div className="flex items-start gap-3 bg-orange-50 border border-orange-200 rounded-lg px-4 py-3">
                    ...
                    <div className="flex flex-col">
                        ...
                        <span className="text-xs text-gray-600">提出いただいた画像は登録のみに使用し、それ以外の目的で使用しません。</span>
                    </div>
                </div>
            </div>
...
export default CastRegisterDirectPage;
