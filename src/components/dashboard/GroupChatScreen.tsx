...
const GroupChatScreen: React.FC<GroupChatScreenProps> = ({
  ...
  const [selectedCastTarget, setSelectedCastTarget] = useState<{
    id: number;
    nickname: string;
  } | null>(null);
    ...
    try {
      ...
      if (showGiftModal && selectedGiftCategory) {
        const selectedGift = gifts.find(
          (gift) => gift.id === selectedGiftCategory
        );
        if (selectedGift) {
  ...
  return (
    <div className="min-h-screen max-w-md mx-auto flex flex-col relative pt-14 pb-28">
      ...
      <div
        className="flex-1 overflow-y-auto px-4 py-2 space-y-4 scrollbar-hidden relative"
      ...
      >
        ...
        {messages.map((message, index) => {
          ...
          const showDate =
            index === 0 ||
            !dayjs(message.created_at).isSame(
              dayjs(messages[index - 1]?.created_at),
              "day"
            );
            ...
            return (
              <div key={message.id}>
                ...
                <div
                  ...
                  style={
                    isAccepted ? { opacity: 0.6, pointerEvents: "none" } : {}
                  }
                >
                  <div
                  ...
                  >
                    ...
                    <div
                      className={`bg-orange-500 text-white rounded-lg px-4 py-3 max-w-[80%] text-sm shadow-md relative`}
                    >
                      <div>
                        日程：
                        {proposal.date
            ...
            <div key={message.id}>
              ...
              >
                ...
                >
                  ...
                  <div
                    className={`rounded-lg px-3 py-2 max-w-full break-all ${
                      isOwnMessage
                  ...
                  >
                    ...
                    {message.message && (
                      <div className="whitespace-pre-wrap max-w-full break-all">
                        {message.message}
                      </div>
                    )}
                  </div>
        ...
        {previewImageUrl && (
          ...
          >
            <div
              className="relative bg-primary rounded-lg shadow-2xl p-2"
              ...
              <div className="absolute left-0 right-0 bottom-full mb-1 bg-white rounded-md shadow-lg max-h-48 overflow-y-auto z-20">
                {participants
                  ...
                  .filter(
                    (p: any) =>
                      !mentionQuery ||
                      (p.nickname || "").toLowerCase().includes(mentionQuery)
                  )
                  .map((cast: any) => (
                    <button
                      key={`mention-${cast.id}`}
                      ...
                      onClick={() => {
                        const replaced = input.replace(
                          /(^|\s)@(\S*)$/,
                          (_m, prefix) => `${prefix}@${cast.nickname} `
                        );
                        setInput(replaced.trimStart());
                        setMentionOpen(false);
                        setMentionQuery("");
                        setSelectedCastTarget({
                          id: cast.id,
            ...
            <div className="bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg p-4 mb-6 shadow-lg">
              <div className="space-y-3">
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2">
                    {selectedProposal.people?.replace(/名$/, "")}人
                  </span>
                </div>
                <div className="flex items-center">
                ...
                <div className="flex items-center">
                  ...
                  <span className="ml-2">
                    {selectedProposal.totalPoints?.toLocaleString()}P
                  </span>
                </div>
                <div className="flex items-center">
                  <span className="text-orange-200 text-sm">
                    （延長：{selectedProposal.extensionPoints?.toLocaleString()}
                    P / 15分）
                  </span>
                </div>
              </div>
                  ...
                  try {
                    ...
                    await updateReservation(reservationId, {
                      scheduled_at: selectedProposal.date
                        ? new Date(selectedProposal.date).toISOString()
                        : undefined,
                      duration: selectedProposal.duration
                        ? parseInt(selectedProposal.duration as string, 10)
                        : undefined,
                    });
                ...
                onClick={async () => {
                  ...
                  try {
                    const payload: any = {
                      ...
                      message:
                        input.trim() || `@${selectedCastTarget.nickname}`,
                    };
