...
import {
  createFreeCall,
  createFreeCallReservation,
  fetchRanking,
  getGuestChats,
  getCastCountsByLocation,
  getCastList,
  getPaymentInfo,
  getGuestProfileById,
} from "../../services/api";
...
const massageIntensityOptions = ["弱め", "普通", "強め"];
...
const conversationOptions = ["多め", "普通", "少なめ", "無し"];

const oilScentOptions = ["無香料", "イランイラン", "柑橘系", "ラベンダー"];
    ...
    <div className="flex items-center justify-center gap-2 py-4">
      {steps.map((label, idx) => (
        <div key={label} className="flex items-center">
          ...
          {idx < steps.length - 1 && (
            <div className="w-8 h-1 bg-gradient-to-r from-secondary to-gray-300 mx-1 rounded" />
          )}
        </div>
      ))}
    </div>
    ...
    const loadAreas = async () => {
      try {
        ...
        const combinedSet = new Set(
          unique.map((p) => `${p.name}/${p.prefecture}`)
        );
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-gradient-to-b from-primary via-primary to-secondary pb-8">
      ...
      <div className="flex items-center px-4 pt-2 pb-2">
        <button
          onClick={onBack}
          className="mr-2 text-2xl text-white hover:text-secondary cursor-pointer transition-colors p-2 rounded-full hover:bg-white/10"
        >
          <ChevronLeft />
        </button>
      ...
      <div className="px-4 mt-4">
        ...
        <div className="flex gap-2 mb-6 flex-wrap">
          {timeOptions.map((opt) => {
            const displayText =
              opt === "それ以外" && selectedTime.includes("時間後")
                ? selectedTime
                : opt;
            const isSelected =
              selectedTime === opt ||
            ...
            return (
              <button
                key={opt}
                className={`px-4 py-2 rounded-full border shadow-sm font-semibold transition-all duration-200 ${
                  isSelected
                    ? "bg-secondary text-white border-secondary scale-105"
                    : "bg-primary text-white border-gray-700 hover:bg-secondary/20 hover:scale-105"
                }`}
          ...
          >
            {(() => {
              ...
              const match = areaOptions.find(
                (o) =>
                  o.name === selectedArea ||
                  `${o.name}/${o.prefecture}` === selectedArea
              );
              return match
                ? `${match.name}/${match.prefecture}`
                : selectedArea || "エリアを選択";
            })()}
          </button>
        </div>
      ...
      <div className="px-4 mt-4">
        ...
        <div className="bg-white/10 rounded-lg p-4 border border-secondary shadow-sm">
          {classOptions.map((opt, idx) => (
            <div
              key={opt.name}
              className="flex items-center mb-4 last:mb-0 justify-between"
            >
              <div className="flex flex-col min-w-[110px]">
                ...
                <span className="text-xs text-white ml-6 mt-0.5">
                  {opt.price.toLocaleString()} P / 30分
                </span>
              </div>
              <div className="flex items-center">
                <button
                  className={`w-8 h-8 rounded-full border flex items-center justify-center text-2xl transition-all duration-200 ${
                    counts[idx] === 0
                      ? "border-gray-800 text-gray-700 bg-gray-900 cursor-not-allowed"
                      : "border-secondary text-white bg-primary hover:bg-secondary/20 hover:scale-110"
                  }`}
                  onClick={() =>
                    counts[idx] > 0 &&
                    setCounts(
                      counts.map((v, i) => (i === idx ? Math.max(0, v - 1) : v))
                    )
                  }
                ...
                <span
                  className={`w-4 text-center font-bold mx-1 ${
                    counts[idx] > 0 ? "text-white" : "text-gray-400"
                  }`}
                >
                  {counts[idx]}
                </span>
                <button
                  ...
                  onClick={() =>
                    setCounts(counts.map((v, i) => (i === idx ? v + 1 : v)))
                  }
                >
                  <Plus />
                </button>
          ...
          <div className="flex items-center mt-2">
            ...
            <span className="ml-auto font-bold text-white">
              合計：<span className="text-white">{total}人</span>
            </span>
          </div>
        </div>
      </div>
      ...
      <div className="max-w-md mx-auto px-4 pt-8 pb-24">
        <button
          className="w-full bg-secondary text-white py-3 rounded-lg font-bold text-lg hover:bg-red-700 transition-all shadow-lg"
          onClick={onNext}
        >
          次に進む
        </button>
...
function OrderDetailConditionsScreen({
  onBack,
  onNext,
  selectedSituations,
  setSelectedSituations,
  selectedCastTypes,
  setSelectedCastTypes,
  selectedCastSkills,
  setSelectedCastSkills,
  selectedMassageIntensity,
  setSelectedMassageIntensity,
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-gradient-to-b from-primary via-primary to-secondary pb-8">
      ...
      <div className="px-4 pt-2 pb-2 flex items-center">
        <button
          onClick={onBack}
          className="mr-2 text-2xl text-white hover:text-secondary transition-colors p-2 rounded-full cursor-pointer"
        >
          <ChevronLeft />
        </button>
        <span className="text-xl font-bold text-white">
          その他詳細条件{" "}
          <span className="bg-secondary text-xs px-2 py-1 rounded align-middle text-white">
            任意
          </span>
        </span>
      </div>
      ...
      <div className="px-4 mt-8">
        ...
        <div className="flex flex-wrap gap-2">
          {ageOptions.map((opt) => (
            <button
              key={opt}
              className={`px-4 py-1 rounded-full border shadow-sm font-semibold transition-all duration-200 ${
                selectedSituations.includes(opt)
                  ? "bg-secondary border-secondary text-white scale-105"
                  : "bg-primary border-gray-700 text-white hover:bg-secondary/20 hover:scale-105"
              }`}
              onClick={() =>
                toggle(selectedSituations, setSelectedSituations, opt)
              }
            >
              {opt}
            </button>
        ...
        <div className="flex flex-wrap gap-2">
          {castTypeOptions.map((opt) => (
            <button
              key={opt}
              className={`px-4 py-1 rounded-full border shadow-sm font-semibold transition-all duration-200 ${
                selectedCastTypes.includes(opt)
                  ? "bg-secondary border-secondary text-white scale-105"
                  : "bg-primary border-gray-700 text-white hover:bg-secondary/20 hover:scale-105"
              }`}
              onClick={() =>
                toggle(selectedCastTypes, setSelectedCastTypes, opt)
              }
            >
              {opt}
            </button>
        ...
        <div className="flex flex-wrap gap-2">
          {tiredAreasOptions.map((opt) => (
            <button
              key={opt}
              className={`px-4 py-1 rounded-full border shadow-sm font-semibold transition-all duration-200 ${
                selectedCastSkills.includes(opt)
                  ? "bg-secondary border-secondary text-white scale-105"
                  : "bg-primary border-gray-700 text-white hover:bg-secondary/20 hover:scale-105"
              }`}
              onClick={() =>
                toggle(selectedCastSkills, setSelectedCastSkills, opt)
              }
            >
              {opt}
            </button>
...
function PishattoCallScreen({
  ...
  const [locationCastCounts, setLocationCastCounts] = useState<{
    [key: string]: number;
  }>({});
    ...
    const fetchLocationsAndCounts = async () => {
      try {
        ...
        const prefecturesData =
          await locationService.getPrefecturesByLocation();
        ...
        setError(
          "場所の読み込みに失敗しました。しばらく待ってから再度お試しください。"
        );
        setLocations([]);
  ...
  const [locationToPrefectures, setLocationToPrefectures] = useState<
    ...
    <div className="min-h-screen bg-gradient-to-b from-primary via-primary to-secondary flex flex-col pb-20">
      ...
      <div className="fixed max-w-md mx-auto left-0 right-0 bg-gradient-to-r from-primary to-blue-900 backdrop-blur-md border-b border-white/10 shadow-lg">
        <div className="flex items-center px-4 pt-4 pb-2">
          <button
            onClick={onBack}
            className="mr-3 text-2xl text-white hover:text-secondary cursor-pointer transition-colors p-2 rounded-full"
          >
            <ChevronLeft />
          </button>
              ...
              <div className="mt-8">
                <div className="bg-white/10 rounded-2xl p-6 border border-white/20 backdrop-blur-sm">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 max-h-64 overflow-y-auto">
                    {locationToPrefectures[selectedLocation].map(
                      (prefecture, idx) => (
                        <div
                          key={idx}
                          className={`text-sm px-3 py-2 rounded-lg text-center truncate transition-all duration-200 hover:scale-105 cursor-pointer ${
                          ...
                          onClick={() =>
                            setSelectedPrefecture((prev) =>
                              prev === prefecture ? null : prefecture
                            )
                          }
                        >
                          {prefecture}
                  ...
                  <div className="text-center mt-4">
                    <div className="text-white/60 text-sm">
                      合計 {locationToPrefectures[selectedLocation].length}{" "}
                      エリア
                    </div>
                  </div>
                </div>
      ...
      try {
        ...
        if (selectedPrefecture) {
          ...
          list = list.filter((c: any) =>
            matchesPrefecture(c.residence, selectedPrefecture)
          );
        } else {
          ...
          const matchesLocation = (
            ...
            console.log(
              `Checking residence: "${residence}" (normalized: "${normResidence}") against location: "${location}" (normalized: "${normLocation}")`
            );
            ...
            if (normResidence.includes("/")) {
              ...
              const matches =
                residencePart.includes(normLocation) ||
                normLocation.includes(residencePart);
              console.log(
                `Residence has '/' format. Parts: [${parts.join(
                  ", "
                )}]. Residence part: "${residencePart}". Matches: ${matches}`
              );
              return matches;
            }
            ...
            const matches =
              normResidence.includes(normLocation) ||
              normLocation.includes(normResidence);
            console.log(`Residence format. Matches: ${matches}`);
            return matches;
          };
          list = list.filter((c: any) =>
            matchesLocation(c.residence, selectedLocation)
          );
        }
        ...
        setError(
          "キャストの読み込みに失敗しました。しばらく待ってから再度お試しください。"
        );
        setCasts([]);
      } finally {
        setLoading(false);
      ...
      <div className="bg-gradient-to-r from-primary to-blue-900 backdrop-blur-md border-b border-white/10 shadow-lg">
        <div className="flex items-center justify-between px-4 pt-4 pb-2">
          <div className="flex items-center">
            <button
              onClick={onBack}
              className="mr-3 text-2xl text-white hover:text-secondary transition-colors p-2 rounded-full cursor-pointer"
            >
              <ChevronLeft />
            </button>
            <div>
              ...
              <div className="w-full">
                <div className="relative bg-gradient-to-b from-white/20 to-white/5 rounded-3xl overflow-hidden shadow-2xl transition-all duration-300 border border-white/20 hover:border-white/30">
                  ...
                  <div className="p-8">
                    ...
                    <div
                      className="flex items-center justify-between bg-gradient-to-r from-white/15 to-white/10 hover:bg-secondary/20 text-white px-6 py-4 rounded-2xl mb-6 shadow-lg cursor-pointer transition-all duration-200"
                      onClick={() => {
                      ...
                      <div className="flex items-center">
                        ...
                        <div className="flex-1">
                          ...
                          <div className="text-sm opacity-90 leading-relaxed">
                            {currentCast.profile_text ||
                              "すぐにいきます!よろしくお願いいたします✨"}
                          </div>
                        </div>
                      </div>
                    </div>
...
function FreeCallReservationCompletionModal({
  isOpen,
  onClose,
  onNavigateToMessage,
}: {
...
function OrderFinalConfirmationScreen({
  ...
  const durationHours =
    customDurationHours ||
    (selectedDuration.includes("以上")
      ? 4
      : selectedDuration === "1分"
      ? 1 / 60
      : Number(selectedDuration.replace("時間", "")));
  const baseCost =
    (18000 * counts[0] * durationHours * 60) / 30 +
    (15000 * counts[1] * durationHours * 60) / 30 +
    (12000 * counts[2] * durationHours * 60) / 30;
  const nightTimeFee = isNightTime
    ? nightTimeFeePerHour * durationHours * (counts[0] + counts[1] + counts[2])
    : 0;
  ...
  const handleReservation = async () => {
    ...
    try {
      ...
      const response = await createFreeCallReservation({
        ...
        details: `フリーコール: VIP:${counts[1]}人, ロイヤルVIP:${
          counts[0]
        }人, プレミアム:${
          counts[2]
        }人, 合計ポイント: ${totalCost.toLocaleString()}P, シチュ: ${selectedSituations.join(
          ","
        )}, タイプ: ${selectedCastTypes.join(
          ","
        )}, スキル: ${selectedCastSkills.join(",")}`,
      ...
      setReservationMessage(
        `予約が完了しました (${response.points_deducted?.toLocaleString()}P 消費)`
      );
  ...
  return (
    <div className="max-w-md mx-auto min-h-screen bg-gradient-to-b from-primary via-primary to-secondary pb-24">
      ...
      <div className="flex items-center px-4 pt-2 pb-2">
        <button
          onClick={onBack}
          className="mr-2 text-2xl cursor-pointer text-white hover:text-secondary transition-colors p-2 rounded-full"
        >
          <ChevronLeft />
        </button>
        ...
        <div className="bg-white/10 rounded-lg p-4 border border-white/20 shadow-sm">
            ...
            <div className="ml-auto text-right">
              ...
              <div className="text-xs text-white/70">
                {scheduledTime
                  ...
                  .replace(/\//g, "-")}
                {isNightTime && (
                  <span className="text-orange-300 ml-1">(深夜)</span>
                )}
              </div>
            </div>
          </div>
          ...
          {selectedAges.length > 0 && (
            <div className="text-white text-sm mb-1">
              年代: {selectedAges.join(", ")}
            </div>
          )}
          {selectedTypes.length > 0 && (
            <div className="text-white text-sm mb-1">
              キャストタイプ: {selectedTypes.join(", ")}
            </div>
          )}
          {selectedPressures.length > 0 && (
            <div className="text-white text-sm mb-1">
              指圧: {selectedPressures.join(", ")}
            </div>
          )}
          {selectedTiredAreas.length > 0 && (
            <div className="text-white text-sm mb-1">
              お疲れ箇所: {selectedTiredAreas.join(", ")}
            </div>
          )}
          {selectedConversations.length > 0 && (
            <div className="text-white text-sm mb-1">
              会話: {selectedConversations.join(", ")}
            </div>
          )}
          {selectedOils.length > 0 && (
            <div className="text-white text-sm mb-1">
              オイルの香り: {selectedOils.join(", ")}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="px-4 mt-8">
        <div className="bg-white/10 rounded-lg p-4 border border-secondary shadow-sm">
          <div className="flex justify-between text-sm mb-1 text-white">
            ...
            <span>
              {((18000 * counts[0] * durationHours * 60) / 30).toLocaleString()}
              P
            </span>
          </div>
          <div className="flex justify-between text-sm mb-1 text-white">
            ...
            <span>
              {((15000 * counts[1] * durationHours * 60) / 30).toLocaleString()}
              P
            </span>
          </div>
          <div className="flex justify-between text-sm mb-1 text-white">
            ...
            <span>
              {((12000 * counts[2] * durationHours * 60) / 30).toLocaleString()}
              P
            </span>
          </div>
          ...
          {isNightTime && (
            <div className="flex justify-between text-sm mt-2 text-white">
              <span className="text-orange-300">
                深夜料金 ({nightTimeFeePerHour.toLocaleString()}P/時間)
              </span>
              <span className="text-orange-300">
                +{nightTimeFee.toLocaleString()}P
              </span>
            </div>
          )}
          ...
          {isNightTime && (
            <div className="text-orange-300 text-xs mt-2 text-center">
              深夜時間帯 (12:00 AM - 6:00 AM) のため、
              {nightTimeFeePerHour.toLocaleString()}
              P/時間の深夜料金が適用されます
            </div>
          )}
        </div>
        ...
        {reservationMessage && (
          <div
            className={`text-center mt-2 text-sm ${
              reservationMessage.includes("不足")
                ? "text-red-400"
                : "text-white"
            }`}
          >
      ...
      <div className="bg-gradient-to-b from-primary to-blue-900 border border-white/20 rounded-3xl w-full max-w-2xl max-h-[90vh] shadow-2xl overflow-hidden">
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 mb-2">
                        <span
                          className={`text-xs font-bold ${getCategoryStyles(
                            "プレミアム"
                          )}`}
                        >
                          プレミアム
        ...
        <div className="bg-white/5 px-6 py-4 border-t border-white/10">
          <div className="flex items-center justify-between">
            <div className="text-white/70 text-sm">
              合計:{" "}
              <span className="text-white font-semibold">{casts.length}人</span>{" "}
              のキャスト
            </div>
            <button
              onClick={onClose}
...
const CallScreen: React.FC<CallScreenProps> = ({
  ...
  const [selectedMassageIntensity, setSelectedMassageIntensity] =
    useState<string>("");
  const [selectedConversation, setSelectedConversation] = useState<string>("");
  const [selectedOilScent, setSelectedOilScent] = useState<string>("");
  const [customDurationHours, setCustomDurationHours] = useState<number | null>(
    null
  );
  ...
  const handleFreeCall = async () => {
    ...
    try {
      ...
      const hours = selectedDuration.includes("以上")
        ? 4
        : selectedDuration === "1分"
        ? 1 / 60
        : Number(selectedDuration.replace("時間", ""));
      ...
      const requestData = {
        ...
        details: `Free call - VIP:${counts[1]}人, ロイヤルVIP:${
          counts[0]
        }人, プレミアム:${counts[2]}人, ${
          meetingLocation ? ` / 住所:${meetingLocation}` : ""
        }, ${reservationName ? ` / お名前:${reservationName}` : ""}`,
        time: selectedTime,
  ...
  if (showStepRequirement)
    return (
      <StepRequirementScreen
        onBack={() => {
          setShowStepRequirement(false);
          setTimeout(refreshSetupStatuses, 0);
        }}
      />
    );
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-primary via-primary to-secondary pb-20">
      ...
      <div
        className={`bg-gradient-to-r from-primary to-blue-900 px-4 pb-4 ${
          hasRegisteredCard && verificationStatus === "success"
            ? "pt-24"
            : "pt-4"
        } flex flex-col gap-3 border-b border-white/10 shadow-lg`}
      >
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
          ...
          <div className="flex items-center mb-4">
            <div className="flex -space-x-2 mr-4">
              <img
                ...
                alt="VIP"
                className="w-8 h-8 rounded-full border-2 border-secondary shadow-lg"
              />
              <img
                src="assets/avatar/woman.png"
                alt="Premium"
        ...
        <div className="bg-gradient-to-b from-white/15 to-white/5 rounded-2xl p-6 border border-white/10 shadow-xl hover:shadow-2xl transition-all duration-300 hover:scale-[1.02]">
          ...
          <div className="flex items-center mb-4">
            <img
              src="assets/avatar/woman.png"
              alt="cast"
              className="w-8 h-8 rounded-full border-2 border-secondary shadow-lg mr-4"
            />
        ...
        <div className="p-8 pt-6">
          <div className="flex gap-3">
            ...
            <button
              ...
              className={`flex-1 py-4 rounded-2xl transition-all duration-200 font-semibold shadow-lg ${
                loading || !area
                  ? "bg-gray-600 text-gray-300 cursor-not-allowed"
      ...
      <div className="bg-gradient-to-b from-primary to-blue-900 border border-white/20 rounded-3xl w-full max-w-md shadow-2xl overflow-hidden animate-fade-in">
        ...
        <div className="p-6">
          ...
          <div className="bg-white/10 rounded-2xl p-4 mb-6 border border-white/20">
            ...
            <div className="text-center">
              <div className="text-white text-3xl font-bold mb-2">
                {occurrencePoints.toLocaleString()}P
              </div>
          ...
          <div className="flex gap-3">
            ...
            <button className="flex-1 bg-gradient-to-r from-secondary to-red-500 text-white py-3 rounded-xl hover:from-red-500 hover:to-red-600 transition-all duration-200 font-semibold shadow-lg">
              予約する
            </button>
          </div>
        </div>
