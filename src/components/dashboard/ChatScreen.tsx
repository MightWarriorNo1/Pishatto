...
const ChatScreen: React.FC<ChatScreenProps> = ({ chatId, onBack }) => {
  ...
  const acceptedFromDb = useMemo(() => {
    ...
    (messages || []).forEach((m: any) => {
      try {
        const parsed =
          typeof m.message === "string" ? JSON.parse(m.message) : null;
        if (
          parsed &&
          parsed.type === "proposal_accept" &&
          typeof parsed.proposalMsgId === "number"
        ) {
  ...
  useEffect(() => {
    ...
    setAcceptedProposals((prev) => {
      const merged = Array.from(
        new Set([...(prev || []), ...Array.from(acceptedFromDb)])
      );
      return merged;
    });
  }, [acceptedFromDb]);
  ...
  useEffect(() => {
    ...
    setDeniedProposals((prev) => {
      const merged = Array.from(
        new Set([...(prev || []), ...Array.from(deniedFromDb)])
      );
      return merged;
    });
  }, [deniedFromDb]);
    ...
    const fetchMessages = async () => {
      ...
      try {
        ...
        if (e.response) {
          ...
          setFetchError(
            `メッセージの取得に失敗しました: ${e.response.status} ${e.response.statusText}`
          );
        } else if (e.request) {
  ...
  useEffect(() => {
    ...
    getGuestReservations(user.id)
      .then(setGuestReservations)
      .catch(() => setGuestReservations([]));
  }, [user?.id]);
  ...
  useEffect(() => {
    ...
    const hasProposals = messages.some((msg) => {
      try {
        const parsed =
          typeof msg.message === "string" ? JSON.parse(msg.message) : null;
  ...
  console.log(
    "ChatScreen: Setting up useChatMessages for chatId:",
    chatId,
    "userId:",
    user?.id
  );
  ...
  useChatMessages(
    ...
    (message) => {
      ...
      if (!isUserLoaded) {
        console.log(
          "ChatScreen: User not loaded, but processing message anyway to avoid losing it"
        );
      ...
      try {
        ...
        if (message.message && typeof message.message === "string") {
          if (
            message.message.includes("承認") ||
            message.message.includes("accepted") ||
            message.message.includes("承認済み") ||
            message.message.includes("accepted")
          ) {
            shouldRefreshReservations = true;
          }
      ...
      setMessages((prev) => {
        ...
        console.log(
          "ChatScreen: Message sender_guest_id:",
          message.sender_guest_id
        );
        console.log(
          "ChatScreen: Message sender_cast_id:",
          message.sender_cast_id
        );
        ...
        const filtered = prev.filter((m) => {
          ...
          if (
            ...
            ((m.sender_guest_id &&
              ...
              (m.sender_cast_id &&
                message.sender_cast_id &&
                m.sender_cast_id === message.sender_cast_id))
          ) {
        ...
        console.log(
          "ChatScreen: New messages count after adding:",
          newMessages.length
        );
        return newMessages;
      });
  ...
  const handleOpenCamera = async () => {
    ...
    try {
      ...
      setCameraError(
        "カメラを利用できません。ブラウザの設定やアクセス権限を確認してください。"
      );
    }
  };
  ...
  const handleSend = async () => {
    if ((input.trim() || attachedFile) && !sending && user) {
      ...
      if (!isMessageNotificationEnabled) {
        setSendError(
          "メッセージ通知が無効になっています。設定で有効にしてください。"
        );
        return;
      }
      ...
      try {
        ...
        if (
          ...
          "ギフト送信に必要なポイントが不足しています"
        ) {
          setSendError(
            `ポイントが不足しています。必要: ${Number(
              e.response.data.required_points
            ).toLocaleString()}P、所持: ${Number(
              e.response.data.available_points
            ).toLocaleString()}P`
          );
        } else {
  ...
  return (
    <div className="bg-gradient-to-b from-primary via-primary to-secondary min-h-screen flex flex-col relative">
      ...
      <div className="fixed max-w-md mx-auto left-0 right-0 items-center flex px-4 py-3 border-b border-secondary bg-primary h-16">
        <button
          onClick={onBack}
          className="mr-2 hover:text-secondary cursor-pointer"
        >
          <ChevronLeft size={30} />
        </button>
        <img
          src={
            castInfo?.avatar
              ? getFirstAvatarUrl(castInfo.avatar)
          ...
          (sortedMessages || []).map((msg, idx) => {
            ...
            const prevDate =
              prev && prev.created_at
                ...
                : "";
            const isSentByGuest =
              user && String(msg.sender_guest_id) === String(user.id);
            ...
            const senderAvatar =
              ...
              "/assets/avatar/female.png";
            const senderName =
              msg.guest?.nickname || msg.cast?.nickname || "ゲスト/キャスト";
            ...
            try {
              const parsed =
                typeof msg.message === "string"
                  ...
                  : null;
              console.log("ChatScreen: Checking for proposal in message:", {
                msgId: msg.id,
                parsed,
                message: msg.message,
              });
            ...
            if (proposal) {
              ...
              const isProposalFromGuest =
                msg.sender_guest_id && !msg.sender_cast_id;
              const proposalPosition = isProposalFromGuest
                ? "justify-end"
                : "justify-start";
              const alignment = isProposalFromGuest
                ? "items-end"
                : "items-start";
                ...
                <React.Fragment key={msg.id || `p-${idx}`}>
                  ...
                  <div className={`flex ${proposalPosition} mb-4`}>
                    <div className={`flex flex-col ${alignment} max-w-[80%]`}>
                      <div
                        className={`bg-orange-600 text-white rounded-lg px-4 py-3 text-sm shadow-md relative w-full ${
                          canClickProposal
                      ...
                      >
                        ...
                        <div>
                          （延長：{proposal.extensionPoints?.toLocaleString()}P
                          / 15分）
                        </div>
                        {isAccepted && (
              ...
              try {
                const parsed =
                  typeof msg.message === "string"
                    ? JSON.parse(msg.message)
                    : null;
                console.log("ChatScreen: Message parsing result:", {
                  msgId: msg.id,
                  parsed,
                  message: msg.message,
                });
                if (
                  ...
                  console.log(
                    "ChatScreen: Hiding proposal_accept/proposal_reject message"
                  );
                  hide = true;
                } else if (parsed && parsed.type === "system") {
                  console.log(
                    "ChatScreen: System message, target:",
                    parsed.target
                  );
                  if (parsed.target !== "guest") {
                    console.log(
                      "ChatScreen: Hiding system message not targeted to guest"
                    );
                    hide = true;
                  } else {
                  ...
                  console.log(
                    "ChatScreen: Proposal message detected, should be visible"
                  );
                }
              } catch (e) {
            ...
            return (
              <React.Fragment key={msg.id || idx}>
                ...
                <div
                  className={
                    isSent ? "flex justify-end mb-4" : "flex justify-start mb-4"
                  }
                >
                  {!isSent && (
                    <img
                      src={
                        senderAvatar
                          ? getFirstAvatarUrl(senderAvatar)
                          : "/assets/avatar/female.png"
                      }
                      alt="avatar"
                    ...
                    {(() => {
                      ...
                      try {
                        ...
                        if (parsed && parsed.type === "system") {
                          ...
                          return (
                            <div
                              className={`max-w-full break-all whitespace-pre-wrap ${
                                isSent
                      ...
                      return (
                        ...
                        >
                          {(() => {
                            const giftObj = msg.gift_id
                              ? msg.gift ||
                                (Array.isArray(gifts)
                                  ? gifts.find((g: any) => g.id === msg.gift_id)
                                  : null)
                              : null;
                              ...
                              <div className="mb-1">
                                <div className="flex items-center">
                                  ...
                                  <span className="ml-2 text-xs text-primary font-bold">
                                    {typeof giftObj.points === "number"
                                      ? giftObj.points.toLocaleString()
                                      : Number(
                                          giftObj.points || 0
                                        ).toLocaleString()}
                                    P
      ...
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-primary border-t border-secondary flex flex-col px-4 py-2 z-20">
        {sendError && (
          <div className="text-red-400 text-xs mb-1">{sendError}</div>
        )}
        {/* Image preview */}
        {imagePreview && (
        ...
        <div
          className="flex items-center w-full relative gap-2 flex-wrap"
          ref={inputBarRef}
        >
          <button
            className={`${
        ...
        <div className="fixed inset-0 z-50 bg-black bg-opacity-60">
          <div className="fixed left-0 right-0 bottom-0 bg-primary rounded-t-2xl shadow-lg p-6 flex flex-col items-center border-t border-secondary w-full max-w-md mx-auto animate-slide-up">
            ...
            <div className="grid grid-cols-4 gap-4 mb-4">
              {gifts
                .filter((g) => g.category === selectedGiftCategory)
                .map((gift) => {
                  return (
                    <button
                      key={gift.id}
                    ...
                    >
                      ...
                      <span className="text-xs text-yellow-300 font-bold">
                        {Number(gift.points).toLocaleString()}P
                      </span>
                    </button>
                  );
                })}
            ...
            <button
              className="text-white mt-2 hover:text-red-700 transition-all duration-200 font-medium"
          ...
          <div className="bg-primary rounded-2xl shadow-lg p-6 flex flex-col items-center min-w-[320px] max-w-[90vw]">
            ...
            <div className="flex flex-col items-center mb-4">
              ...
              <span className="text-lg font-bold text-white mb-1">
                {selectedGift.name || selectedGift.label}
              </span>
              <span className="text-yellow-300 font-bold mb-2">
                {Number(selectedGift.points).toLocaleString()}P
              </span>
              {selectedGift.description && (
                <span
                  className="text-white text-sm whitespace-pre-line mb-2 text-center px-4"
                  style={{ maxWidth: 320 }}
                >
                  {selectedGift.description}
                </span>
            ...
            {giftSendError && (
              <div className="mb-4 w-full px-3 py-2 rounded-lg bg-red-500/20 border border-red-400 text-red-200 text-sm text-center">
                {giftSendError}
              </div>
            )}
              ...
              <button
                ...
                onClick={async () => {
                  ...
                  try {
                    ...
                    if (!giftObj) {
                      giftObj =
                        gifts.find((g: any) => g.id === selectedGift.id) ||
                        selectedGift;
                      sent.gift = {
                        id: giftObj.id,
            ...
            <div className="mb-4 text-black">
              <div>
                ...
                {selectedProposal.date
                  ? new Date(selectedProposal.date)
                      .toLocaleString("ja-JP", {
                        year: "numeric",
                        month: "2-digit",
                        day: "2-digit",
              ...
              <div>
                （延長：
                {selectedProposal.extensionPoints?.toLocaleString() || "0"}P /
                15分）
              </div>
            </div>
                ...
                onClick={async () => {
                  ...
                  if (
                    ...
                    setProposalActionError(
                      "Missing required data for proposal acceptance"
                    );
                    return;
                  }
                  ...
                  try {
                    ...
                    const duration = parseInt(
                      selectedProposal.duration?.toString() || "0",
                      10
                    );
                    if (isNaN(duration) || duration <= 0) {
                      setProposalActionError(
                        "Invalid duration for proposal acceptance"
                      );
                      return;
                    }
                    ...
                    try {
                      await ProposalService.sendConfirmationMessages(
                        chatId,
                        {
                          guest_id: user.id,
                          date: selectedProposal.date,
                      ...
                      console.error(
                        "Failed to send confirmation messages:",
                        error
                      );
                    }
                    ...
                    if (
                      e?.response?.data?.error === "Insufficient guest points"
                    ) {
                      errorMessage = "ゲストのポイントが不足しています。";
                    } else if (
                      e?.response?.data?.message?.includes(
                        "ポイントが不足しており、自動支払いも失敗しました"
                      )
                    ) {
                      errorMessage =
                        "ゲストのポイントが不足しており、自動支払いも失敗しました。ゲストにカード登録をお願いしてください。";
